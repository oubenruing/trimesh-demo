...
// TriMesh *TriMesh::read_mem(const int & addr, const size_t & len)
// {
...
// 	if (f!=NULL)
//   {
//     fwrite ((char*)addr , sizeof(char), len, f);
//     fclose (f);
//   }
...
static bool read_obj(FILE *f, TriMesh *mesh)
{
	...
	Miniball<3,float> mb;
	mb.check_in(mesh->vertices.begin(), mesh->vertices.end());
	mb.build();
	point center = mb.center();
	float r = sqrt(mb.squared_radius());
	int nv = mesh->vertices.size();
	mesh->scaleParam.at(0) = center.x;
	mesh->scaleParam.at(1) = center.y;
	mesh->scaleParam.at(2) = center.z;
	mesh->scaleParam.at(3) = r;
